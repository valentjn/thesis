    ...
    mF(F)
{
    ...
    Real*** Fmap = AllocateMap3<Real>(mXBound, mYBound, mZBound,
        const_cast<Real*>(mF));
    ...
    Real*** FXY = GetFXY(Fmap);
    Real*** FXZ = GetFXZ(Fmap);
    Real*** FYZ = GetFYZ(Fmap);

    // Construct third-order derivatives.
    Real*** FXYZ = GetFXYZ(Fmap);
    ...
    Deallocate3<Real>(FY);
    Deallocate3<Real>(FZ);
    Deallocate3<Real>(FXY);
    Deallocate3<Real>(FXZ);
    Deallocate3<Real>(FYZ);
    Deallocate3<Real>(FXYZ);
    DeallocateMap3<Real>(Fmap);
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
template <typename Real>
Real*** IntpAkimaUniform3<Real>::GetFX(Real*** F)
{
    Real*** slope = Allocate3<Real>(mXBound + 3, mYBound, mZBound);
    ...
    for (iz = 0; iz < mZBound; ++iz)
    {
        for (iy = 0; iy < mYBound; ++iy)
        {
            ...
            {
                slope[iz][iy][ix + 2] =
                    (F[iz][iy][ix + 1] - F[iz][iy][ix]) * invDX;
            }

            slope[iz][iy][1] = ((Real)2)*slope[iz][iy][2] - slope[iz][iy][3];
            slope[iz][iy][0] = ((Real)2)*slope[iz][iy][1] - slope[iz][iy][2];
            slope[iz][iy][mXBound + 1] =
                ((Real)2)*slope[iz][iy][mXBound] - slope[iz][iy][mXBound - 1];
            slope[iz][iy][mXBound + 2] =
                ((Real)2)*slope[iz][iy][mXBound + 1] - slope[iz][iy][mXBound];
        }
    }

    Real*** FX = Allocate3<Real>(mXBound, mYBound, mZBound);
    for (iz = 0; iz < mZBound; ++iz)
    {
        for (iy = 0; iy < mYBound; ++iy)
        {
    ...
    Deallocate3<Real>(slope);
    return FX;
}
//----------------------------------------------------------------------------
template <typename Real>
Real*** IntpAkimaUniform3<Real>::GetFY(Real*** F)
{
    Real*** slope = Allocate3<Real>(mYBound + 3, mXBound, mZBound);
    ...
    for (iz = 0; iz < mZBound; ++iz)
    {
        for (ix = 0; ix < mXBound; ++ix)
        {
            ...
            {
                slope[iz][ix][iy + 2] =
                    (F[iz][iy + 1][ix] - F[iz][iy][ix]) * invDY;
            }

            slope[iz][ix][1] = ((Real)2)*slope[iz][ix][2] - slope[iz][ix][3];
            slope[iz][ix][0] = ((Real)2)*slope[iz][ix][1] - slope[iz][ix][2];
            slope[iz][ix][mYBound + 1] =
                ((Real)2)*slope[iz][ix][mYBound] - slope[iz][ix][mYBound - 1];
            slope[iz][ix][mYBound + 2] =
                ((Real)2)*slope[iz][ix][mYBound + 1] - slope[iz][ix][mYBound];
        }
    }

    Real*** FY = Allocate3<Real>(mXBound, mYBound, mZBound);
    for (iz = 0; iz < mZBound; ++iz)
    {
        for (ix = 0; ix < mXBound; ++ix)
        {
...
//----------------------------------------------------------------------------
template <typename Real>
Real*** IntpAkimaUniform3<Real>::GetFZ(Real*** F)
{
    Real*** slope = Allocate3<Real>(mZBound + 3, mXBound, mYBound);
    ...
    for (iy = 0; iy < mYBound; ++iy)
    {
        for (ix = 0; ix < mXBound; ++ix)
        {
            ...
            {
                slope[iy][ix][iz + 2] =
                    (F[iz + 1][iy][ix] - F[iz][iy][ix]) * invDZ;
            }

            slope[iy][ix][1] = ((Real)2)*slope[iy][ix][2] - slope[iy][ix][3];
            slope[iy][ix][0] = ((Real)2)*slope[iy][ix][1] - slope[iy][ix][2];
            slope[iy][ix][mZBound + 1] =
                ((Real)2)*slope[iy][ix][mZBound] - slope[iy][ix][mZBound - 1];
            slope[iy][ix][mZBound + 2] =
                ((Real)2)*slope[iy][ix][mZBound + 1] - slope[iy][ix][mZBound];
        }
    }

    Real*** FZ = Allocate3<Real>(mXBound, mYBound, mZBound);
    for (iy = 0; iy < mYBound; ++iy)
    {
        for (ix = 0; ix < mXBound; ++ix)
        {
...
//----------------------------------------------------------------------------
template <typename Real>
Real*** IntpAkimaUniform3<Real>::GetFXY(Real*** F)
{
    ...
    Real*** FXY = Allocate3<Real>(mXBound, mYBound, mZBound);
...
//----------------------------------------------------------------------------
template <typename Real>
Real*** IntpAkimaUniform3<Real>::GetFXZ(Real*** F)
{
    int xBoundM1 = mXBound - 1;
    ...
    Real*** FXZ = Allocate3<Real>(mXBound, mYBound, mZBound);
...
//----------------------------------------------------------------------------
template <typename Real>
Real*** IntpAkimaUniform3<Real>::GetFYZ(Real*** F)
{
    int yBoundM1 = mYBound - 1;
    int zBoundM1 = mZBound - 1;
    int iy0 = yBoundM1, iy1 = iy0 - 1, iy2 = iy1 - 1;
    ...
    Real*** FYZ = Allocate3<Real>(mXBound, mYBound, mZBound);
...
//----------------------------------------------------------------------------
template <typename Real>
Real*** IntpAkimaUniform3<Real>::GetFXYZ(Real*** F)
{
    int xBoundM1 = mXBound - 1;
    int yBoundM1 = mYBound - 1;
    int zBoundM1 = mZBound - 1;
    ...
    Real*** FXYZ = Allocate3<Real>(mXBound, mYBound, mZBound);
    ...
    for (iz = 0; iz <= 2; ++iz)
    {
        for (iy = 0; iy <= 2; ++iy)
        {
            for (ix = 0; ix <= 2; ++ix)
            {
                ...
                FXYZ[0][0][xBoundM1] += mask*
                    F[iz][iy][xBoundM1 - ix];

                FXYZ[0][yBoundM1][0] += mask*
                    F[iz][yBoundM1 - iy][ix];

                FXYZ[0][yBoundM1][xBoundM1] += mask*
                    F[iz][yBoundM1 - iy][xBoundM1 - ix];

                FXYZ[zBoundM1][0][0] += mask*
                    F[zBoundM1 - iz][iy][ix];

                FXYZ[zBoundM1][0][xBoundM1] += mask*
                    F[zBoundM1 - iz][iy][xBoundM1 - ix];

                FXYZ[zBoundM1][yBoundM1][0] += mask*
                    F[zBoundM1 - iz][yBoundM1 - iy][ix];

                FXYZ[zBoundM1][yBoundM1][xBoundM1] += mask*
                    F[zBoundM1 - iz][yBoundM1 - iy][xBoundM1 - ix];
            }
        }
    }
        ...
        for (iz = 0; iz <= 2; ++iz)
        {
            for (iy = 0; iy <= 2; ++iy)
            {
                for (ix = 0; ix <= 2; ++ix)
                {
                    ...
                    FXYZ[0][yBoundM1][ix0] += mask*
                        F[iz][yBoundM1 - iy][ix0 + ix - 1];

                    FXYZ[zBoundM1][0][ix0] += mask*
                        F[zBoundM1 - iz][iy][ix0 + ix - 1];

                    FXYZ[zBoundM1][yBoundM1][ix0] += mask*
                        F[zBoundM1 - iz][yBoundM1 - iy][ix0 + ix - 1];
                }
            }
        }
    }
        ...
        for (iz = 0; iz <= 2; ++iz)
        {
            for (iy = 0; iy <= 2; ++iy)
            {
                for (ix = 0; ix <= 2; ++ix)
                {
                    ...
                    FXYZ[0][iy0][xBoundM1] += mask*
                        F[iz][iy0 + iy - 1][xBoundM1 - ix];

                    FXYZ[zBoundM1][iy0][0] += mask*
                        F[zBoundM1 - iz][iy0 + iy - 1][ix];

                    FXYZ[zBoundM1][iy0][xBoundM1] += mask*
                        F[zBoundM1 - iz][iy0 + iy - 1][xBoundM1 - ix];
                }
            }
        }
    }
        ...
        for (iz = 0; iz <= 2; ++iz)
        {
            for (iy = 0; iy <= 2; ++iy)
            {
                for (ix = 0; ix <= 2; ++ix)
                {
                    ...
                    FXYZ[iz0][0][xBoundM1] += mask*
                        F[iz0 + iz - 1][iy][xBoundM1 - ix];

                    FXYZ[iz0][yBoundM1][0] += mask*
                        F[iz0 + iz - 1][yBoundM1 - iy][ix];

                    FXYZ[iz0][yBoundM1][xBoundM1] += mask*
                        F[iz0 + iz - 1][yBoundM1 - iy][xBoundM1 - ix];
                }
            }
        }
    }
            ...
            for (iz = 0; iz <= 2; ++iz)
            {
                for (iy = 0; iy <= 2; ++iy)
                {
                    for (ix = 0; ix <= 2; ++ix)
                    {
                        ...
                        FXYZ[0][iy0][ix0] += mask*
                            F[iz][iy0 + iy - 1][ix0 + ix - 1];

                        FXYZ[zBoundM1][iy0][ix0] += mask*
                            F[zBoundM1 - iz][iy0 + iy - 1][ix0 + ix - 1];
                    }
                }
            }
        }
            ...
            for (iz = 0; iz <= 2; ++iz)
            {
                for (iy = 0; iy <= 2; ++iy)
                {
                    for (ix = 0; ix <= 2; ++ix)
                    {
                        ...
                        FXYZ[iz0][0][ix0] += mask*
                            F[iz0 + iz - 1][iy][ix0 + ix - 1];

                        FXYZ[iz0][yBoundM1][ix0] += mask*
                            F[iz0 + iz - 1][yBoundM1 - iy][ix0 + ix - 1];
                    }
                }
            }
        }
            ...
            for (iz = 0; iz <= 2; ++iz)
            {
                for (iy = 0; iy <= 2; ++iy)
                {
                    for (ix = 0; ix <= 2; ++ix)
                    {
                        ...
                        FXYZ[iz0][iy0][0] += mask*
                            F[iz0 + iz - 1][iy0 + iy - 1][ix];

                        FXYZ[iz0][iy0][xBoundM1] += mask*
                            F[iz0 + iz - 1][iy0 + iy - 1][xBoundM1 - ix];
                    }
                }
            }
        }
...
//----------------------------------------------------------------------------
template <typename Real>
void IntpAkimaUniform3<Real>::GetPolynomials(Real*** F, Real*** FX,
    Real*** FY, Real*** FZ, Real*** FXY, Real*** FXZ, Real*** FYZ,
    Real*** FXYZ)
{
    int xBoundM1 = mXBound - 1;
    int yBoundM1 = mYBound - 1;
    int zBoundM1 = mZBound - 1;
    mPoly = Allocate3<Polynomial>(xBoundM1, yBoundM1, zBoundM1);
