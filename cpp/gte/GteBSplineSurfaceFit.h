    ...
    mControlData(numControls0 * numControls1)
{
    ...
    for (dim = 0; dim < 2; ++dim)
    {
        ...
        input.uniqueKnots = new UniqueKnot<Real>[input.numUniqueKnots];
    ...
    for (dim = 0; dim < 2; dim++)
    {
        ATMat[dim] = Allocate2<Real>(mNumSamples[dim], mNumControls[dim]);
