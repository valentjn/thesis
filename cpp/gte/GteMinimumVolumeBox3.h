...
// File Version: 1.0.11 (2015/05/18)
...
private:
    ...
    ExtrudeRectangle SmallestRectangle(int i0, int i1,
...
MinimumVolumeBox3<InputType, ComputeType>::MinimumVolumeBox3(
    unsigned int numThreads, bool threadProcessEdges)
    :
...
void MinimumVolumeBox3<InputType, ComputeType>::ProcessFaces(
    ETManifoldMesh const& mesh, Box& minBox)
{
    ...
    if (mNumThreads > 1 && numFaces >= mNumThreads)
    {
        ...
        for (unsigned int t = 0; t < mNumThreads; ++t)
        {
            ...
            if (minBox.volume == mNegOne
                || localMinBox[t].volume < minBox.volume)
            {
