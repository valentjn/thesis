...
template <typename Real>
GenerateMeshUV<Real>::UVComputeModel::UVComputeModel(DX11Engine* inEngine,
    unsigned int inNumThreads,
    std::function<void(unsigned int)> const* inProgress)
    :
    ComputeModel(inEngine, inNumThreads),
    progress(inProgress)
{
}
...
//----------------------------------------------------------------------------
template <typename Real>
GenerateMeshUV<Real>::GenerateMeshUV(
    std::shared_ptr<UVComputeModel> const& cmodel)
    :
    mCModel(cmodel),
    mNumVertices(0),
    mVertices(nullptr),
...
void GenerateMeshUV<Real>::TopologicalVertexDistanceTransform()
{
    ...
    while (currFront.size() > 0)
    {
        ...
        std::copy(nextFront.begin(), nextFront.end(),
...
{
    HLSLDefiner definer;
    definer.SetInt("NUM_X_THREADS", 8);
    definer.SetInt("NUM_Y_THREADS", 8);
    if (std::numeric_limits<Real>::max() == std::numeric_limits<float>::max())
    {
        definer.SetString("Real", "float");
        definer.SetString("Real2", "float2");
    }
    else
    {
        definer.SetString("Real", "double");
        definer.SetString("Real2", "double2");
    }

    mSolveSystem.reset(ShaderFactory::CreateCompute(
        "GenerateMeshUVs::SolveSystem", gGenerateMeshUVsHLSLString,
        definer));
    ...
    mBoundBuffer.reset(new ConstantBuffer(4 * sizeof(int), false));
    int* data = mBoundBuffer->Get<int>();
    data[0] = xElements;
    data[1] = yElements;
    data[2] = mNumBoundaryEdges;
    data[3] = numInputs;
    mSolveSystem->Set("Bounds", mBoundBuffer);

    mVGBuffer.reset(new StructuredBuffer(
        (unsigned int)mVertexGraph.size(), 3 * sizeof(int)));
    Memcpy(mVGBuffer->GetData(), &mVertexGraph[0],
        mVGBuffer->GetNumBytes());
    mSolveSystem->Set("vertexGraph", mVGBuffer);

    mVGDBuffer.reset(new StructuredBuffer(
        (unsigned int)mVertexGraphData.size(),
        sizeof(std::pair<int, Real>)));
    Memcpy(mVGDBuffer->GetData(), &mVertexGraphData[0],
        mVGDBuffer->GetNumBytes());
    mSolveSystem->Set("vertexGraphData", mVGDBuffer);

    mOVBuffer.reset(new StructuredBuffer(
        (unsigned int)mOrderedVertices.size(), sizeof(int)));
    Memcpy(mOVBuffer->GetData(), &mOrderedVertices[0],
        mOVBuffer->GetNumBytes());
    mSolveSystem->Set("orderedVertices", mOVBuffer);

    for (int j = 0; j < 2; ++j)
    {
        mTCoordsBuffer[j].reset(new StructuredBuffer(mNumVertices,
            sizeof(Vector2<Real>)));
        mTCoordsBuffer[j]->SetUsage(Resource::SHADER_OUTPUT);
        Memcpy(mTCoordsBuffer[j]->GetData(), mTCoords,
            mTCoordsBuffer[j]->GetNumBytes());
    }
    ...
    for (unsigned int i = 1; i <= numIterations; ++i)
    {
        ...
        mSolveSystem->Set("inTCoords", mTCoordsBuffer[0]);
        mSolveSystem->Set("outTCoords", mTCoordsBuffer[1]);
    ...
    Memcpy(mTCoords, mTCoordsBuffer[0]->GetData(),
        mTCoordsBuffer[0]->GetNumBytes());
}
