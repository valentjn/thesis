...
protected:
    ...
    bool GetAndRemoveInsertionPolygon(int i,
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType> inline
Vector2<InputType> const* Delaunay2<InputType, ComputeType>::GetVertices()
    const
{
    return mVertices;
}
...
bool Delaunay2<InputType, ComputeType>::Update(int i)
{
    ...
    else
    {
        ...
        {
            Triangle const* t = element.second;
            for (int j = 0; j < 3; ++j)
            {
                if (!t->T[j])
                {
