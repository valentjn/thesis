...
private:
    ...
    // needed for the input tree and it converts InputType vertices to
    // ComputeType values.
    int InitializeFromTree(Tree const& tree);
    ...
    // certain of a correct result, choose ComputeType to be BSNumber.  The
    // InputType points are convertex to ComputeType points on demand; the
    // mIsConverted array keeps track of which input points have been
    // converted.
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
TriangulateEC<InputType, ComputeType>::TriangulateEC(int numPoints,
...
TriangulateEC<InputType, ComputeType>::GetTriangles() const
{
    return mTriangles;
}
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
bool TriangulateEC<InputType, ComputeType>::operator()()
{
    mTriangles.clear();
    if (mPoints)
    {
    ...
    Polygon const& polygon)
{
    ...
    if (mPoints)
    {
        ...
        InitializeVertices(polygon.numIndices, polygon.indices);
        DoEarClipping(polygon.numIndices, polygon.indices);
        return true;
    }
    else
    {
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
bool TriangulateEC<InputType, ComputeType>::operator()(Polygon const& outer,
    Polygon const& inner)
{
    mTriangles.clear();
    if (mPoints)
    {
        // Two extra elements are needed to duplicate the endpoints of the
        // edge introduced to combine outer and inner polygons.
        ...
        // triangulation calls.
        for (int i = 0; i < outer.numIndices; ++i)
        {
            int index = outer.indices[i];
            if (!mIsConverted[index])
            {
                mIsConverted[index] = true;
                for (int j = 0; j < 2; ++j)
        ...
        for (int i = 0; i < inner.numIndices; ++i)
        {
            int index = inner.indices[i];
            if (!mIsConverted[index])
            {
                mIsConverted[index] = true;
                for (int j = 0; j < 2; ++j)
        ...
        // Combine the outer polygon and the inner polygon into a simple
        // polygon by inserting two edges connecting mutually visible
        // vertices, one from the outer polygon and one from the inner
        // polygon.
        ...
        // albeit one with coincident edges.
    ...
    Polygon const& outer, std::vector<Polygon> const& inners)
{
    ...
    if (mPoints)
    {
        ...
        // endpoints of the edges introduced to combine outer and inner
        // polygons.
        ...
        // Combine the outer polygon and the inner polygons into a simple
        // polygon by inserting two edges per inner polygon connecting
        // mutually visible vertices.
        int nextElement = mNumPoints;  // The next available element.
        std::map<int, int> indexMap;
        std::vector<int> combined;
        if (!ProcessOuterAndInners(nextElement, outer, inners, indexMap,
            combined))
        {
            // An unexpected condition was encountered.
            return false;
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
bool TriangulateEC<InputType, ComputeType>::operator()(Tree const& tree)
{
    mTriangles.clear();
    if (mPoints)
    {
        ...
        while (treeQueue.size() > 0)
        {
            ...
            if (numChildren == 0)
            {
                ...
                numVertices = outer->polygon.numIndices;
                indices = outer->polygon.indices;
                InitializeVertices(numVertices, indices);
                DoEarClipping(numVertices, indices);
            }
            else
            {
                ...
                std::vector<int> combined;
                ProcessOuterAndInners(nextElement, outer->polygon, inners,
                    indexMap, combined);
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
void TriangulateEC<InputType, ComputeType>::Delete(Tree*& tree,
    bool deletePolygons)
{
    if (tree)
    {
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
void TriangulateEC<InputType, ComputeType>::InitializeVertices(
    int numVertices, int const* indices)
{
    mVertices.clear();
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
void TriangulateEC<InputType, ComputeType>::DoEarClipping(int numVertices,
    int const* indices)
{
    ...
    while (bRemoveAnEar)
    {
        ...
        mTriangles.push_back(
        { { V(iVPrev).index, V(mEFirst).index, V(iVNext).index } });
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
bool TriangulateEC<InputType, ComputeType>::CombinePolygons(int nextElement,
    Polygon const& outer, Polygon const& inner, std::map<int, int>& indexMap,
    std::vector<int>& combined)
{
    // Locate the inner-polygon vertex of maximum x-value, call this vertex M.
    ComputeType xmax = mComputePoints[inner.indices[0]][0];
    int xmaxIndex = 0;
    for (int i = 1; i < inner.numIndices; ++i)
    {
        ComputeType x = mComputePoints[inner.indices[i]][0];
        if (x > xmax)
        {
            xmax = x;
            xmaxIndex = i;
    ...
    Vector2<ComputeType> M = mComputePoints[inner.indices[xmaxIndex]];

    // Find the edge whose intersection Intr with the ray M+t*(1,0) minimizes
    // the ray parameter t >= 0.
    ComputeType const cmax =
        (ComputeType)std::numeric_limits<InputType>::max();
    ...
    ComputeType s = cmax;
    ComputeType t = cmax;
    for (i0 = outer.numIndices - 1, i1 = 0; i1 < outer.numIndices; i0 = i1++)
    {
        // Consider only edges for which the first vertex is below (or on) the
        // ray and the second vertex is above (or on) the ray.
        Vector2<ComputeType> diff0 = mComputePoints[outer.indices[i0]] - M;
        if (diff0[1] > zero)
        {
            continue;
        }
        Vector2<ComputeType> diff1 = mComputePoints[outer.indices[i1]] - M;
        if (diff1[1] < zero)
        {
            continue;
        }
        ...
        else if (t == intr[0])
        {
            ...
            // <outer[i0],outer[i1]>.
            Vector2<ComputeType> shared = mComputePoints[outer.indices[i1]];
            ...
            diff0 = mComputePoints[outer.indices[i0]] - shared;
            diff1 = mComputePoints[outer.indices[other]] - shared;
    ...
    if (endMin == -1)
    {
        ...
        if (v0min < 0 || v1min < 0)
        {
            LogError("Is this an invalid nested polygon?");
            return false;
        }
        ...
        int pIndex;
        if (mComputePoints[outer.indices[v0min]][0] >
            mComputePoints[outer.indices[v1min]][0])
        {
            sTriangle[0] = mComputePoints[outer.indices[v0min]];
            sTriangle[1] = intr;
            sTriangle[2] = M;
            pIndex = v0min;
        }
        ...
        {
            sTriangle[0] = mComputePoints[outer.indices[v1min]];
            sTriangle[1] = M;
            sTriangle[2] = intr;
            pIndex = v1min;
        }
        ...
        for (int i = 0; i < outer.numIndices; ++i)
        {
            ...
            int curr = outer.indices[i];
            int prev =
                outer.indices[(i + outer.numIndices - 1) % outer.numIndices];
            int next = outer.indices[(i + 1) % outer.numIndices];
    ...
    // between them.
    combined.resize(outer.numIndices + inner.numIndices + 2);
    int cIndex = 0;
    for (int i = 0; i <= maxCosIndex; ++i, ++cIndex)
    {
        combined[cIndex] = outer.indices[i];
    }

    for (int i = 0; i < inner.numIndices; ++i, ++cIndex)
    {
        int j = (xmaxIndex + i) % inner.numIndices;
        combined[cIndex] = inner.indices[j];
    }

    int innerIndex = inner.indices[xmaxIndex];
    ...
    int outerIndex = outer.indices[maxCosIndex];
    ...
    for (int i = maxCosIndex + 1; i < outer.numIndices; ++i, ++cIndex)
    {
        combined[cIndex] = outer.indices[i];
    }
    return true;
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
bool TriangulateEC<InputType, ComputeType>::ProcessOuterAndInners(
    int& nextElement, Polygon const& outer,
    std::vector<Polygon> const& inners, std::map<int, int>& indexMap,
    std::vector<int>& combined)
{
    ...
    {
        int numIndices = inners[p].numIndices;
        int const* indices = inners[p].indices;
    ...
    std::vector<int> currentOuter(outer.numIndices);
    std::copy(outer.indices, outer.indices + outer.numIndices,
        currentOuter.begin());
    Polygon currentPolygon = { (int)currentOuter.size(), &currentOuter[0] };
    for (int p = numInners - 1; p >= 0; --p)
    {
        Polygon const& polygon = inners[pairs[p].second];
        std::vector<int> currentCombined;
        if (!CombinePolygons(nextElement, currentPolygon, polygon,
            indexMap, currentCombined))
        {
            return false;
        }
        currentOuter = currentCombined;
        currentPolygon.numIndices = (int)currentOuter.size();
        currentPolygon.indices = &currentOuter[0];
        nextElement += 2;
    }

    for (auto index : currentOuter)
    {
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
void TriangulateEC<InputType, ComputeType>::RemapIndices(
    std::map<int, int> const& indexMap)
{
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
int TriangulateEC<InputType, ComputeType>::InitializeFromTree(
    Tree const& tree)
{
    // Use a breadth-first search to process the outer-inners pairs of the
    // tree of nested polygons.
    ...
    while (treeQueue.size() > 0)
    {
        ...
        {
            int index = outer->polygon.indices[i];
            if (!mIsConverted[index])
            {
                mIsConverted[index] = true;
                for (int j = 0; j < 2; ++j)
        ...
        for (int c = 0; c < numChildren; ++c)
        {
            ...
            {
                int index = inner->polygon.indices[i];
                if (!mIsConverted[index])
                {
                    mIsConverted[index] = true;
                    for (int j = 0; j < 2; ++j)
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType> inline
typename TriangulateEC<InputType, ComputeType>::Vertex&
TriangulateEC<InputType, ComputeType>::V(int i)
{
    return mVertices[i];
}
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
bool TriangulateEC<InputType, ComputeType>::IsConvex(int i)
{
    Vertex& vertex = V(i);
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
bool TriangulateEC<InputType, ComputeType>::IsEar(int i)
{
    Vertex& vertex = V(i);
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
void TriangulateEC<InputType, ComputeType>::InsertAfterC(int i)
{
    if (mCFirst == -1)
    {
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
void TriangulateEC<InputType, ComputeType>::InsertAfterR(int i)
{
    if (mRFirst == -1)
    {
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
void TriangulateEC<InputType, ComputeType>::InsertEndE(int i)
{
    if (mEFirst == -1)
    {
        // Add the first ear.
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
void TriangulateEC<InputType, ComputeType>::InsertAfterE(int i)
{
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
void TriangulateEC<InputType, ComputeType>::InsertBeforeE(int i)
{
    Vertex& first = V(mEFirst);
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
void TriangulateEC<InputType, ComputeType>::RemoveV(int i)
{
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
int TriangulateEC<InputType, ComputeType>::RemoveE(int i)
{
...
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
void TriangulateEC<InputType, ComputeType>::RemoveR(int i)
{
