    ...
    mXSpacing(xSpacing)
{
    ...
    mXMax = mXMin + mXSpacing * static_cast<Real>(this->mQuantity - 1);

    // Compute slopes.
    Real invDX = ((Real)1) / mXSpacing;
    std::vector<Real> slope(this->mQuantity + 3);
    int i, ip1, ip2;
    for (i = 0, ip1 = 1, ip2 = 2; i < this->mQuantity - 1; ++i, ++ip1, ++ip2)
    {
    ...
    slope[this->mQuantity + 1] =
        ((Real)2)*slope[this->mQuantity] - slope[this->mQuantity - 1];
    slope[this->mQuantity + 2] =
        ((Real)2)*slope[this->mQuantity + 1] - slope[this->mQuantity];

    // Construct derivatives.
    std::vector<Real> FDer(this->mQuantity);
    ...
    for (i = 0, ip1 = 1; i < this->mQuantity - 1; ++i, ++ip1)
    {
