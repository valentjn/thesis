    ...
    mF(F)
{
    ...
    Real** Fmap = AllocateMap2<Real>(mXBound, mYBound, const_cast<Real*>(mF));
    ...
    Deallocate2<Real>(FX);
    Deallocate2<Real>(FY);
    Deallocate2<Real>(FXY);
    DeallocateMap2<Real>(Fmap);
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
template <typename Real>
Real** IntpAkimaUniform2<Real>::GetFX(Real** F)
{
    Real** slope = Allocate2<Real>(mXBound + 3, mYBound);
    ...
    for (iy = 0; iy < mYBound; ++iy)
    {
        ...
        slope[iy][mXBound + 1] =
            ((Real)2)*slope[iy][mXBound] - slope[iy][mXBound - 1];
        slope[iy][mXBound + 2] =
            ((Real)2)*slope[iy][mXBound + 1] - slope[iy][mXBound];
    }

    Real** FX = Allocate2<Real>(mXBound, mYBound);
    for (iy = 0; iy < mYBound; ++iy)
    {
    ...
    Deallocate2<Real>(slope);
    return FX;
}
//----------------------------------------------------------------------------
template <typename Real>
Real** IntpAkimaUniform2<Real>::GetFY(Real** F)
{
    Real** slope = Allocate2<Real>(mYBound + 3, mXBound);
    ...
    for (ix = 0; ix < mXBound; ++ix)
    {
        ...
        slope[ix][mYBound + 1] =
            ((Real)2)*slope[ix][mYBound] - slope[ix][mYBound - 1];
        slope[ix][mYBound + 2] =
            ((Real)2)*slope[ix][mYBound + 1] - slope[ix][mYBound];
    }

    Real** FY = Allocate2<Real>(mXBound, mYBound);
    for (ix = 0; ix < mXBound; ++ix)
    {
        for (iy = 0; iy < mYBound; ++iy)
        {
...
//----------------------------------------------------------------------------
template <typename Real>
Real** IntpAkimaUniform2<Real>::GetFXY(Real** F)
{
    ...
    Real** FXY = Allocate2<Real>(mXBound, mYBound);
...
//----------------------------------------------------------------------------
template <typename Real>
void IntpAkimaUniform2<Real>::GetPolynomials(Real** F, Real** FX, Real** FY,
    Real** FXY)
{
    int xBoundM1 = mXBound - 1;
    int yBoundM1 = mYBound - 1;
    mPoly = Allocate2<Polynomial>(xBoundM1, yBoundM1);
