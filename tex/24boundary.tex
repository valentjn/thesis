...
\begin{definition}[regular sparse grid with coarse boundary]
  ...
  \begin{equation}
    ...
    \begin{split}
      ...
      &\big(\{\*l \in \natz^d \setminus \nat^d \mid
      \normone{\vecmax(\*l, \*1)} \le n-b+1\} \cup \{\*0\}\big).
    \end{split}
  \end{equation}
...
\begin{restatable}[invariant of \cref{alg:coarseBoundary}]{proposition}{%
  ...
  \begin{equation}
    ...
    \begin{split}
      ...
      &\hphantom{{}={}} \big(\{\*l \in \natz^t \setminus \nat^t \mid
      \normone{\vecmax(\*l, \*1)} \le n-d+t-b+1\} \cup \{\*0\}\big).
    \end{split}
  \end{equation}
\end{restatable}
\begin{proof}
...
\end{proof}
\begin{shortcorollary}[correctness of \cref{alg:coarseBoundary}]
...
\begin{algorithm}
  ...
  \caption[%
    ...
    to the sparse grid $\coarseregsgset{n}{d}{b}$ with coarse boundary.
...
$\lnot(\*l \ge \*1)$, i.e., if the basis function corresponds to a
boundary point.
...
\usenotation{Ëmod}
The resulting \term{modified hat functions} $\bspl[\modified]{l,i}{1}$
...
\begin{SCfigure}
  ...
  \caption[%
    ...
    Modified hierarchical hat functions $\basis[\modified]{l',i'}{1}$
