...
\begin{equation}
  ...
  := \regsgset{n}{d}{} \cap \openinterval{\ß0, \ß1}
...
\begin{definition}[regular sparse grid with coarse boundary]
  ...
  $\Omega_{n,d}^{\sparse(0)} := \regsgset{n}{d}{}$.
\end{definition}
...
$\regsgset{n}{d}{} = \Omega_{n,d}^{\sparse(0)}$.
...
\newgsymbol{Omegasd}{$\bndry{\Omega}^\sparse$}{%
  Boundary grid points $:= \sgset \setminus \interior{\Omega}^\sparse$
...
\coarseregsgset{n}{d}{b}{} \setminus \interior{\Omega}_{n,d}^{\sparse(b)}$.
