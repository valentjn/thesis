...
\begin{equation}
  ...
  = \{\gp{\ßl,\ßi} \in \regsgset{n}{d}{} \mid \ßl \ge \ß1\},
\end{equation}
...
\begin{equation}
  ...
  \{\gp{\ßl,\ßi} \mid \ßi \in \hiset{\ßl}\},\quad
  ...
  \begin{algorithmic}[1]
    \Function{$\coarselevelset{n}{d}{b}{} =$ computeSGCoarseBoundary}{%
      ...
      \For{$t = 2, \dotsc, d$}
        ...
        \For{$\ßl \in \levelset^{(t-1)}$}
          \If{%
            $\normone{\vecmax(\ßl, \ß1)} \le n - d + t - b$ or
            $\ßl = \ß0$%
          }%
          \label{line:algCoarseBoundary1}
            \State{$\levelset^{(t)} \gets \levelset^{(t)} \cup \{(\ßl, 0)\}$}
            \Comment{add corners}%
...
For the hat function basis $\hierbspl{\ßl,\ßi}{1}{}$,
this is a feasible option if the objective
function $\objfun\colon \clint{\ß0, \ß1} \to \real$
satisfies homogeneous boundary conditions
$\restrictfcn{\objfun}{\bndrydomain{\clint{\ß0, \ß1}}} \equiv 0$,
as $\hierbspl{\ßl,\ßi}{1}{}$ vanishes on the boundary if and only if
