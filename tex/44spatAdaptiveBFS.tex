...
\begin{proof}
  We have $\ns[\mathrm{ext}]{\*l'} \supset \ns[\tift,\mathrm{ext}]{\*l'}$ as
  $\basis[\tift]{\*l',\*i'} \in \ns[\mathrm{ext}]{\*l'}$
...
\begin{theorem}[unique existence of fundamental spline coefficients]
  ...
  $(c_i^p)_{i \in \integer}$ and the corresponding parent function
  $\parentfcn[p,\fs]\colon \real \to \real$ defined by
  $\parentfcn[p,\fs](x) :=
  \sum_{i \in \integer} c_k^p \parentfcn[p](x - i)$
  satisfies
  \begin{equation}
    \exfalarge{\beta_p, \gamma_p \in \posreal}{x \in \real}{
      |\parentfcn[p,\fs](x)|
      \le \beta_p \exp(-\ln(\gamma_p) |x|).
    }
  \end{equation}
\end{theorem}

\begin{proof}
  See Lecture 4, Theorems 1 and 2 in \cite{Schoenberg72Cardinal}.
\end{proof}
