...
use a linear combination $\hftbasis{l',i'}$
...
\begin{equation}
  ...
  \fafalarge{l = 0, \dotsc, l'}{i \in \hiset{l}}{
    \hftbasis{l',i'}(\gp{l,i}) = \kronecker{(l,i)}{(l',i')},
  }
\end{equation}
i.e., $\basis{l',i'}^\hft$ ($l' \in \natz$, $i' = 0, \dotsc, 2^{l'}$)
interpolates the data
$\{(\gp{l',i}, \kronecker{i}{i'}) \mid i = 0, \dotsc, 2^{l'}\}$.
The coefficients $c_{l'',i''}^{l',i'} \in \real$
are, in general, different for each basis function $\hftbasis{l',i'}$.
...
In addition, the transformation $\basis{l',i'} \mapsto \hftbasis{l',i'}$
does not preserve the locality of the support the basis functions.
Consequently, $\hftbasis{l',i'}$ may be globally supported,
which means that we have to evaluate up to $2^{l'} + 1$ basis functions
$\basis{l'',i''}$ when evaluating $\hftbasis{l',i'}$ at a single point
$x \in \clint{0, 1}$.

We call the transformation $\basis{l',i'} \mapsto \hftbasis{l',i'}$
\term{\hftr}.
...
\begin{proposition}[%
  ...
  \begin{equation}
    \regsgspace{n}{d}
    = \regsgspace[\hft]{n}{d}
    := \spn\{\hftbasis{\*l',\*i'} \mid (\*l', \*i') \in \liset\}.
  \end{equation}
\end{proposition}

\begin{proof}
  We have $\regsgspace{n}{d} \supset \regsgspace[\hft]{n}{d}$ as
  $\hftbasis{\*l',\*i'} \in \regsgspace{n}{d}$
  ...
  the functions $\hftbasis{\*l',\*i'}$ ($(\*l', \*i') \in \liset$),
  are linearly independent.
  Let $\surplus{\*l',\*i'} \in \real$ be with
  $\sum_{(\*l', \*i') \in \liset}
  \surplus{\*l',\*i'} \hftbasis{\*l',\*i'} \equiv 0$.
  By evaluating at $\gp{\*l,\*i}$ ($(\*l, \*i) \in \liset$), we obtain
  \begin{equation}
    \sum_{(\*l', \*i') \in \liset}
    \surplus{\*l',\*i'} \hftbasis{\*l',\*i'}(\gp{\*l,\*i}) = 0,\quad
    (\*l, \*i) \in \liset.
  \end{equation}
  ...
  Hence, the functions $\hftbasis{\*l',\*i'}$ ($(\*l', \*i') \in \liset$),
  are linearly independent.
\end{proof}
...
$\parentfcn\colon \clint{0, 1} \to \real$
such that
\begin{equation}
...
that for each basis function $\hftbasis{l',i'}$, we have to
...
with nodal basis functions $\basis{l',i''}$ and arbitrary general
integer indices $i'' \in \integer$:
\begin{equation}
  ...
  \falarge{i \in \hiset{l'}}{
    \tiftbasis{l',i'}(\gp{l',i}) = \kronecker{i}{i'},
  }
\end{equation}
where $l' \in \natz$, $i' = 0, \dotsc, 2^{l'}$, and
$c_{i''}^{l',i'} \in \real$.
Of course, $\basis{l',i''}$ has to be defined for integer indices
$i'' \in \integer$, which is the case for translation-invariant bases
$\basis{l',i''}$, as they can be generalized to $i'' \in \integer$
...
then $\parentfcn^\tift\colon \clint{0, 1} \to \real$ defined by
\begin{equation}
  ...
  c_{i''} := c_{i'+i''}^{l',i'},
\end{equation}
is a parent function of $\tiftbasis{l',i'}$ that satisfies
\begin{equation}
...
\begin{equation}
  ...
  i \in \integer.
\end{equation}
This shows that the transformation $\basis{l',i'} \mapsto \tiftbasis{l',i'}$
...
$\basis{l',i'} \mapsto \tiftbasis{l',i'}$ \term{\tiftr}.
...
\begin{proposition}[%
  ...
  \begin{equation}
    ...
    =: \ns{\*l'}^\mathrm{ext}
    = \ns{\*l'}^{\tift,\mathrm{ext}}
    := \spn\{\tiftbasis{\*l',\*i'} \mid \*i' \in \relindexset{\*l'}\},\quad
    \*l' \in \natz^d.
  \end{equation}
\end{proposition}

\begin{proof}
  We have $\ns{\*l'}^\mathrm{ext} \supset \ns{\*l'}^{\tift,\mathrm{ext}}$ as
  $\tiftbasis{\*l',\*i'} \in \ns{\*l'}^\mathrm{ext}$
  ...
  To prove that
  $\ns{\*l'}^\mathrm{ext} \subset \ns{\*l'}^{\tift,\mathrm{ext}}$,
  we show that the dimensions of the two spaces match.
  Therefore, it suffices to show that
  the functions $\tiftbasis{\*l',\*i'}$ ($\*i' \in \relindexset{\*l'}$),
  are linearly independent.
  Let $\surplus{\*l',\*i'} \in \real$ be with
  $\sum_{\*i' \in \relindexset{\*l'}}
  \surplus{\*l',\*i'} \tiftbasis{\*l',\*i'} \equiv 0$.
  By evaluating at $\gp{\*l',\*i}$ ($\*i \in \relindexset{\*l'}$), we obtain
  \begin{equation}
    0
    = \sum_{\*i' \in \relindexset{\*l'}} \surplus{\*l',\*i'}
    \underbrace{\tiftbasis{\*l',\*i'}(\gp{\*l',\*i})}_{=\kronecker{\*i}{\*i'}}
    = \surplus{\*l',\*i},\quad
    \*i \in \relindexset{\*l'},
  \end{equation}
  i.e., all coefficients $\surplus{\*l',\*i'}$ must vanish.
  Hence, the functions $\tiftbasis{\*l',\*i'}$ ($\*i' \in \relindexset{\*l'}$)
  are linearly independent.
\end{proof}
