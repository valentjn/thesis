  ...
  \begin{subequations}
    \begin{align}
      ...
      \intertext{%
        ...
        equal $\fcnval{\*l,\*i}$ due to the full grid interpolation
    ...
    \end{align}
  \end{subequations}
  We will prove that the sum of the first $\sum$ sign in
  \cref{eq:combiTechniqueSplitSum}
  equals one (see \cref{prop:combiTechniqueOne})
  and that the sum of the second $\sum$ sign
...
It says that the contributions $\fgintp{\*l'}$ of levels $\*l'$
...
as for in \thmref{thm:combiTechnique}.
...
on the nodal space.
In \cref{chap:a0proofs}, we prove the following invariant of the algorithm that
...
shows this exploiting the auxiliary statement of
