...
\begin{algorithm}
  \begin{algorithmic}[1]
    \Function{$\ßy =$ unidirectionalPrinciple}{%
      $\lis$, $(t_1, \dotsc, t_d)$, $\ßu$%
    }
      \State{$\ßy^{(0)} \gets \ßu$}
      \For{$j = 1, \dotsc, d$}
        \State{%
          Define equivalence relation $\sim$ on $\lis$
          as $\ßk \sim \ßk' \iff \ßk_{-t_j} = \ßk'_{-t_j}$%
        }
        \For{$\lis_\mathrm{pole} \in \lis/{\sim}$}
          \State{%
            $(y_\ßk^{(j)})_{\ßk \in \lis_\mathrm{pole}} \gets
            L_{t_j}\Big[(y_\ßk^{(j-1)})_{\ßk \in \lis_\mathrm{pole}}\Big]$%
          }
...
\begin{proof}
  ...
  \begin{align}
    ...
    \intertext{%
      ...
      %= ((L_{t_j})^{-1}
      %\Big[(\surplus{\ßk}^{(j)})_{\ßk \in \lis_\mathrm{pole}}\Big])_{k'_{t_j}}
      %= ((\surplus{\ßk}^{(j-1)})_{\ßk \in \lis_\mathrm{pole}})_{k'_{t_j}}
