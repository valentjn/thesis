...
\begin{figure}
  ...
  \begin{tikzpicture}[>={Stealth[width=1.5mm,length=1.5mm]}]
    \node (input)        at (0,0) {$\vlinin = \vlinout^{(0)}$};
    \node (intermediate) at (4.55,0) {$\vlinout^{(1)}$};
    \node (output)       at (9.1,0) {$\vlinout^{(2)} = \vlinout$};
    \draw[->] (input) -- node[below] {$\linop_1$} (intermediate);
    \draw[->] (intermediate) -- node[below] {$\linop_2$} (output);
    \draw[->] (input) -- (0,-1) -- node[above] {$\linop$} (9.1,-1) -- (output);
  \end{tikzpicture}%
  \caption[%
    ...
    \Function{$\vlinout =$ unidirectionalPrinciple}{%
      ...
      \For{$j = 1, \dotsc, d$}
        \State{%
          ...
          as $\*k \sim \*k' \iff \*k_{-t_j} = \*k'_{-t_j}$%
        }
        \For{$\liset_\mathrm{pole} \in \eqclasses{\liset}{\sim}$}
          \State{%
            $(\linout{\*k}^{(j)})_{\*k \in \liset_\mathrm{pole}} \gets
            \linop_{t_j}
            \Big[(\linout{\*k}^{(j-1)})_{\*k \in \liset_\mathrm{pole}}\Big]$%
          }
...
\begin{proof}
  ...
  \begin{align}
    ...
    \intertext{%
      ...
      %= ((\linop_{t_j})^{-1}
      %\Big[(\surplus{\*k}^{(j)})_{\*k \in \liset_\mathrm{pole}}\Big])_{k'_{t_j}}
      %= ((\surplus{\*k}^{(j-1)})_{\*k \in \liset_\mathrm{pole}})_{k'_{t_j}}
