...
\begin{algorithm}
    ...
    \Function{$\ßy =$ unidirectionalPrinciple}{%
      ...
      \For{$j = 1, \dotsc, d$}
        ...
        \For{$\liset_\mathrm{pole} \in \liset/{\sim}$}
          \State{%
            $(y_\ßk^{(j)})_{\ßk \in \liset_\mathrm{pole}} \gets
            L_{t_j}\Big[(y_\ßk^{(j-1)})_{\ßk \in \liset_\mathrm{pole}}\Big]$%
          }
  ...
  \caption{%
    ...
    the vector $\ßu = (u_\ßk)_{\ßk \in \liset}$ of input data.
    The output is the vector $\ßy = (y_\ßk)_{\ßk \in \liset}$
    of output data.%
  }%
...
\begin{proposition}[%
  ...
  i.e.,
  $T = A^{-1}$,
  $\ßu = (\fcnval{\ßk})_{\ßk \in \liset}$,
  $\ßy = (\surplus{\ßk})_{\ßk \in \liset}$,
