...
\begin{equation}
  ...
  \fa{\*i = \*0, \dotsc, \*2^\*l}{\fgintp{\*l}(\gp{\*l,\*i}) = \objfun(\gp{\*l,\*i})},
\end{equation}
where $\interpcoeff{\*i} \in \real$ and
...
\end{lemma}
\begin{proof}
  Assume that $\interpcoeff{\*i} \in \real$ are chosen in \eqref{eq:interpFullGridMV}
  ...
  \begin{equation}
    ...
    \left(\sum_{i_d=0}^{2^{l_d}} \interpcoeff{\*i} \basis{l_d,i_d}(\gp{l_d,i_d'})\right) \dotsb
  ...
  Repeating this argument for all dimensions, we infer that $\interpcoeff{\*i} = 0$
  for all~$\*i = \*0, \dotsc, \*2^\*l$,
  implying the linear independence of the functions $\basis{\*l,\*i}$.
\end{proof}
The lemma is equivalent to the statement that the coefficients $\interpcoeff{\*i} \in \real$
