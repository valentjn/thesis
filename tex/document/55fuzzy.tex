...
To this end, we modify the Ritter--Novak criterion
to create a grid generation method that is tailored for the
fuzzy extension principle, resulting in \cref{alg:fuzzyRitterNovakMethod}.
...
of $\clint{\*0, \*1}$.
Therefore, we apply the Ritter--Novak criterion twice to
...
The following experiments use $\gamma = 0.1$ as adaptivity.

\begin{algorithm}
  \begin{algorithmic}[1]
    \Function{$\liset = \text{fuzzyRitterNovakMethod}$}{%
      ...
      \While{$\setsize{\liset} < \ngpMax$}
        ...
        \For{$j = 0, \dotsc, m$}
          ...
          \State{%
            ...
            \gp{\*l,\*i} \in \Omega_{\alpha_j} \cap \clint{\*0, \*1}\}$%
          }
          \Comment{set of feasible points}%
          ...
          \State{%
            $(\*l^\ast, \*i^\ast) \gets
            \argmin_{(\*l,\*i) \in \liset_j} \bracket*{
              (r_{\*l,\*i} + 1)^\gamma
    ...
    \EndFunction{}
  \end{algorithmic}
  \caption[Fuzzy Ritter--Novak method]{%
    Fuzzy Ritter--Novak method to generate spatially adaptive sparse grids
  ...
  \label{alg:fuzzyRitterNovakMethod}%
\end{algorithm}
...
the spatially adaptive sparse grids generated by the fuzzy Ritter--Novak
method improve results significantly.
\todo{write text}
\todo{replace figure below with actual plots}
...
\begin{figure}
  ...
  \caption[Fuzzy errors for spatially adaptive sparse grids]{%
    ...
    := \normLtwo{\memfun[\reference]{y} - \memfun[\sparse,\ast]{y}}/
    \normLtwo{\memfun[\reference]{y}}$
    for spatially adaptive sparse grids $\sgset$
    and different objective functions $\objfun$ \emph{(colors)}
