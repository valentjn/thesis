...
one can compute so-called \term{elasticity tensors} $\etensor_q$,
...
\term{(micro-problem)}.
Once all elasticity tensors $\etensor_q$ are known,
...
\begin{subequations}
  ...
  \begin{gather}
    ...
    := \frac{1}{M} \sum_{q=1}^M \dens(\*x^{(q)}).
  \end{gather}
\end{subequations}
Here, $\dens(\*x^{(q)}) \in \clint{0, 1}$ is the
