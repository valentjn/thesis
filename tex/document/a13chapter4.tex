...
\begin{proof}
  ...
  \begin{subequations}
    \begin{align}
      ...
      (\*x_{\range{1}{m}})
      &\hphantom{:}= \sum_{\*i'_{\range{1}{m}}=\*0}^{\*2^{\*l'_{\range{1}{m}}}}
...
\begin{proof}
  ...
  %In the latter case, we trivially obtain $\min\{l'_t, l''_t\} \ge l_t$.
  %Therefore, we assume that $l'_t \not= l''_t$.
  %As $t \notin T_{\*l',\*l''}$, we must have
  i.e., $t \notin T_{L_0}$.
  ...
  \begin{itemize}
    ...
    \begin{subequations}
      \begin{align}
        ...
        &\hphantom{:}= \sum_{l'_{t'}=0}^{l_{t'}^{(j+1)}}
