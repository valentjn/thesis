...
$\compliance[\sparse](\mcp{1}, \dotsc, \mcp{M})$.%
\footnote{%
  ...
  \subcaptionbox{%
    ...
    \begin{tikzpicture}
      ...
      \node[myCircle] (choleskyFactors) at (34mm,0mm) {%
        $
          \begin{matrix}
            \cholfactor[\sparse](\mcp{1}),\\
            \dots,\\
            \cholfactor[\sparse](\mcp{M})
          \end{matrix}
        $%
      };
      \node[myCircle] (elasticityTensors) at (83mm,0mm) {%
        $
          \begin{matrix}
            \etensor[\chol,\sparse](\mcp{1}),\\
            \dots,\\
            \etensor[\chol,\sparse](\mcp{M})
          \end{matrix}
        $%
      };
...
\begin{equation}
  \frac{
    \normLtwoscaled{
      \vphantom{\big(}
      \norm[2]{\etensor({\cdot}) - \etensor[\chol,\sparse]({\cdot})}
    }
  }{
    \normLtwoscaled{
      \vphantom{\big(}
...
\end{equation}
and the corresponding error for $\etensor[\sparse]({\cdot})$
