...
The macro-domains $\domain$ were discretized using
...
(i.e.,
%$50 \times 20 + 20 \times 30 = 1600$
1600 cells for the 2D L-shape,
%$20 \times 20 \times 20 = 8000$
8000 cells for the 3D cantilever, and
%$20 \times 20 \times 10 = 4000$
4000 cells for the 3D center-load).
...
Therefore, the figure show for each macro-cell only
...
B-splines on sparse grids lead to a significant reduction in runtime.
Solving the 2D cantilever scenario with the best-placed sheared cross model
would take 453 days with
exact elasticity tensor evaluations (without surrogates),
...
\begin{figure}
  ...
  \caption[Convergence of the optimality-interpolation gap]{%
    ...
    In addition, the left plot shows the compliance values
    $\compliance[\opt,\ast]$ and $\complianceintp[\opt,\ast]$
...
\end{equation}
Unfortunately, the true optimal parameter combination
...
and errors stemming from types
\ref{item:topoOptErrorMicro} and \ref{item:topoOptErrorRounding}
led to optimizer crashes as it ran into discontinuities,
...
($\complianceintp[\opt,\ast]$ vs. $\complianceintp[\opt,\ast]$).
...
all optimizations run with piecewise linear functions
...
This application shows that good interpolation
(and thus a good quality of the surrogate)
