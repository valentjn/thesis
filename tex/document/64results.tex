...
$\*f\colon \clint{\*0, \*1} \to \real^{m+1}$ is but vector-valued.
...
while the evaluation of the interpolants (online phase) were performed
...
major new contribution in this application.
...
\begin{figure}
  ...
  \\[2mm]%
  \subcaptionbox{%
    $\delta^{\sparse}$ \emph{\textcolor{C0}{(blue)}} and
    $\delta^{\chol,\sparse}$ \emph{\textcolor{C1}{(red)}}
...
B-splines on sparse grids lead to a drastic reduction in runtime.
