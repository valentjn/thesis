...
without boundary points to reduce the sample size
...
For this goal, we create (as in \cite{Valentin18Gradient}) a test scenario
...
as the input of the optimization problem O1 or O2.
...
For the reference forces, we abuse the fact that the reference surrogates
...
As mentioned in \cite{Valentin18Gradient}, spatially adaptive sparse grids
...
but rather use an adaptivity criterion to decide which grid points to
refine iteratively.
