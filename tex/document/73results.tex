...
For this goal, we introduce the following test scenario
...
\begin{enumerate}
  ...
  \item
  Apply O2 with $\forceL(t_3) := \SI{40}{\newton}$ and
  $\tarelbang(t_3) := \ang{60}$ (changed load).
  
  \item
  Apply O2 with $\forceL(t_4) := \SI{100}{\newton}$ and
  $\tarelbang(t_4) := \ang{30}$ (changed load and target angle).
\end{enumerate}
...
\begin{figure}
  ...
  \subcaptionbox{%
    ...
    deviation $|\momentref|$ of the moment from equilibrium.%
  }[73.3mm]{%
