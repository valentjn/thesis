...
the error might steadily become larger backwards in time $t$.
...
a mean number $\ngp_t = \num{12572}$ of policy grid points for $t = 0$.
...
\begin{figure}
  ...
  %\begin{minipage}[b]{110.5mm}%
    \caption[Monte Carlo simulation of the TCP]{%
      ...
      \emph{\textcolor{C2}{(yellow, right axes)}.}%
    }%
    %\vspace*{-6.3mm}%
...
\begin{figure}
  ...
  \caption[Computation times and numbers of iterations for the TCP]{%
    ...
    ``Time per opt.''\ is this time divided by the number $T\ngp$
