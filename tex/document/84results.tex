...
However, as the stock fractions do not change much over time,
...
The latter might be due to vectorization effects.
...
Hierarchical B-splines introduce two major benefits into
the solution of dynamic portfolio choice models.
The first benefit is the smooth objective functions:
...
magnitude worse than in the cubic case).
...
is much higher (e.g., almost seven times as high for $d = 3$) for $p = 1$
than in the cubic case,
...
is one order of magnitude shorter for cubic B-splines.
...
similar, the total optimization time roughly increases by the
factor of four if we do not use exact gradients.
