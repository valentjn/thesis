...
%Another approach, which is based on the \up\punctfix{,}
...
fine-level B-splines $\bspl{\*l',\*i'}{\*p}$ do not vanish
...
have a strictly finer level $l_t$ or
...
\begin{algorithm}
  \begin{algorithmic}[1]
    \Function{$\vlinout = \text{breadthFirstSearch}$}{%
      $\vlinin$, $\liset$%
    }
...
a similar statement can be proved for dimensionally adaptive case.
...
%$\le \*p$ on all hyper-cubes $\clint{\gp{\*l,\*i}, \gp{\*l,\*i+\*1}}$
%($\*i = \*0, \dotsc, \*2^\*l - \*1$) and
