...
However, more sophisticated, algorithm-tailored approaches
are also available \cite{More87Optimization}.
...
For arbitrary runtime (e.g., if the decrease rate of the temperature
is small enough), simulated annealing is guaranteed to find the global
...
The velocity $\*v_{k+1}$ of the next iteration is calculated as
the weighted sum of $\*v_k$,
the vector from $\*x_k$ to the best neighborhood position, and
the vector from $\*x_k$ to the best own position.
To add a stochastic component, the weights are chosen pseudo-randomly.
...
A common choice for the family of stochastic processes and
for the acquisition function are Gaussian processes
%(characterized by the mean function and
%the covariance matrix function)
and upper confidence bounds (which are the sum of the mean
and a multiple of the standard deviation), respectively,
in which case we obtain the GP-UCB method.
...
algorithm; the best choice is the solution of the univariate
\term{line search problem:}
\begin{equation}
  \alpha_k
  = \argmin_{\alpha \in \posreal} \objfun(\*x_k + \alpha \*d_k).
\end{equation}
While there are various line search algorithms,
the most common among these is the \term{Armijo line search algorithm}
\multicite{Nocedal99Numerical,Ulbrich12Nichtlineare},
...
as we have to solve a linear system with system matrix
$\hessian{\*x}{\objfun}(\*x_k)$.
Therefore, often a regularization or damping term $\lambda \eye$
...
Quasi-Newton methods only differ in which solution to choose.
