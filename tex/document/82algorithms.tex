...
\begin{figure}
  ...
  \caption[Scheme of the generation of value function interpolants]{%
    ...
    \texttt{solveValueFunction} repeatedly calls the \texttt{optimize} routine
...
we just use $\valueintp[p]_{t+1}(\state_{t+1}^\mathrm{in})$
for the extrapolated value $\valueintp[p]_{t+1}(\state_{t+1})$.
...
\begin{subequations}
  \begin{align}
    ...
    &= \valueintp[p]_{t+1}(\state_{t+1}^\mathrm{in}) +
    \sum_{s=1}^d
    \partialderiv{\partialdiff{}\stateentry_s}{\valueintp[p]_{t+1}}(
...
Hessian $\hessian{\state}{\valueintp[p]_{t+1}}$,
for which we would need value function values at
$\binom{d}{2} + 2d + 1$ points (which is quadratic in $d$).

\begin{algorithm}
  \begin{algorithmic}[1]
    \Function{$\valueintp[p]_{t+1}(\state_{t+1}) = \texttt{extrapolate}$}{%
      $\valueintp[p]_{t+1}$, $\state_{t+1}$%
    }
      \State{%
        $\state_{t+1}^\mathrm{in}
      ...
      \State{%
        $\valueintp[p]_{t+1}(\state_{t+1})
        \gets \valueintp[p]_{t+1}(\state_{t+1}^\mathrm{in})$%
      }
      \If{type $\not=$ constant}
        \For{$s = 1, \dotsc, d$}
          \State{%
          ...
          \If{$\sigma_s \not= 0$}
            ...
            \ElsIf{type $=$ quadratic}
              ...
              \State{%
                ...
                \gets \text{%
                  ...
                  $\valueintp[p]_{t+1}(\stateentry_s, \*\state_{t+1,-s})$
                  at $D$%
                }$%
              }
            ...
            \State{%
              $\valueintp[p]_{t+1}(\state_{t+1})
              \gets \valueintp[p]_{t+1}(\state_{t+1}) +
              \sigma_s v_s \cdot
              (\stateentry_{t+1,s} - \stateentry_{t+1,s}^\mathrm{in})$%
            }
          \EndIf{}
  ...
  \caption[%
    ...
    Evaluation of $\valueintp[p]_{t+1}$ at $\state_{t+1}$
    by extrapolation.
    Inputs are
    the value function interpolant $\valueintp[p]_{t+1}$
    of the next iteration $t + 1$ and
    the state $\state_{t+1} \in \real^d \setminus \clint{\*0, \*1}$
    at which to evaluate the value function.
    The output is
    the value function value $\valueintp[p]_{t+1}(\state_{t+1})$
    at $\state_{t+1}$.
    Note that $\valueintp[p]_{t+1}(\stateentry_s, \*\state_{t+1,-s})$
    in \cref{line:algFinanceExtrapolateParabola} is short for
    the univariate function $\valueintp[p]_{t+1}(
