...
are the state%
\footnote{%
  We assume that each state variable $\stateentry_{t,s}$ ($s = 1, \dotsc, d$)
...
then the optimal terminal solution is usually
...
Therefore, we have to interpolate $\valuefcn_t$
at the grid points to obtain an interpolant $\valueintp_t$:
\begin{equation}
