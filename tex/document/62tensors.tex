...
macro-problem.
However, the elasticity tensors $\etensor_q$ of all $M$ macro-cells
...
\begin{equation}
\nabla_{\*x^{(q)}} \etensor_q(\*x^{(q)}),\quad
\nabla_{\*x^{(q)}} \dens_q(\*x^{(q)}),\qquad
q = 1, \dotsc, M,
\end{equation}
where $\etensor_q$ is the elasticity tensor and
$\dens_q$ is the density of the $q$-th macro-cell.
