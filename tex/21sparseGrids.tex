...
$i = 0, \dotsc, 2^l - 1$.
The nodal hat function basis and a linear combination are shown in
\cref{fig:nodalHat}.
...
\begin{figure}
  ...
  \hfill%
  \subcaptionbox{%
    Piecewise linear interpolant $f_l$ as a weighted linear combination
...
\end{equation}
As an example, \cref{fig:nodalHat2D} shows a bivariate nodal hat function
$\varphi_{\ßl,\ßi}^1$.
...
\Cref{fig:regularSG} shows a regular sparse grid in two dimensions.
...
which is visualized in \cref{fig:combinationTechnique}, decomposes the
...
\begin{figure}
  ...
  \caption{%
    ...
    sum to form a regular sparse grid of level $n = 3$ in two dimensions.
    The red subspaces are subtracted from the sum of the green subspaces.%
  }
...
values linearly towards the boundary \cite{Pflueger10Spatially}
