...
\newnotationcommand{\stateentry}{x}{xtj}{$\stateentry_{t,j}$}{
  $j$-th state variable $\in \clint{0, 1}$ of time $t$
  ($j = 1, \dotsc, d$)
}
...
\newnotationcommand{\policyentry}{p}{ptj}{$\policyentry_{t,j}$}{
  $j$-th policy variable $\in \real$ of time $t$
  ($j = 1, \dotsc, m_{\policy}$)
}
...
\newnotationcommand{\discrstate}{\*\theta}{Äθt}{$\discrstate_t$}{
  Discrete state variables $\in \discrstdomain$ of time $t$
}
...
\newnotationcommand{\optpolicyintp}{%
  ...
  $\optpolicyintp_t\colon \clint{\*0, \*1} \to \real^{m_{\policy}}$ of time $t$
}
