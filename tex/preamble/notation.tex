...
\newnotationcommand[1]{\stdbasis}{\*e_{#1}}{et}{$\stdbasis{t}$}{
  ...
  $\stdbasis{t} := (0, \dotsc, 0, 1, 0, \dotsc, 0) \in \real^d$
}
...
\newnotationcommand[1]{\normLtwo}{\norm[\Ltwo]{#1}}{ÖnormL2}{%
  ...
  $\Ltwo$ norm
  $\normLtwo{\objfun} := \sqrt{\int_\Omega \objfun(\*x)^2 \diff{}\*x}$
  of a function $\objfun\colon \Omega \to \real$
}

\newcommand*{\normLtwoscaled}[1]{\normscaled[\Ltwo]{#1}}

\newnotationcommand[1]{\normLinfty}{\norm[\Linfty]{#1}}{ÖnormLinfty}{%
  $\normLinfty{\cdot}$%
}{
  $\Linfty$ norm
  $\normLinfty{\objfun} := \max_{\*x \in \Omega} \abs{\objfun(\*x)}$
...
\makecommandnotation{\interiorgrid}{ÄΩs2}{$\interiorsgset$}{
...
\newnotationcommand[2]{\kronecker}{\delta_{#1,#2}}{ÄδAB}{$\kronecker{A}{B}$}{
  Kronecker delta $:= 1$ if $A = B$ and $:= 0$ otherwise
...
\hidenextnotation
\newnotationcommand{\refinetol}{\kappa}{Äκ}{$\refinetol$}{
  Refinement tolerance
}

\hidenextnotation
\newnotationcommand{\error}{\varepsilon}{Äε}{$\error$}{
  Error measure
}
...
\hidenextnotation
\newnotationcommand{\objdomain}{\tilde{\Omega}}{ÄΩtilde}{$\objdomain$}{
  Object domain
}

\hidenextnotation
\newnotationcommand{\densglobal}{\tilde{\varrho}}{Äρ2}{$\densglobal$}{
  Global density fcn.
}
...
\newnotationcommandoptarg{1}{\compliance}{%
  J\ifnotempty{#1}{^{#1}}%
}{Jρ}{$\compliance(\densglobal)$}{
  Compliance fcn.
}

\hidenextnotation
\newnotationcommand[1]{\vol}{\mathrm{vol}(#1)}{volΩ}{$\vol{\objdomain}$}{
  Total volume
}
...
\newnotationcommand[1]{\equielbang}{%
  \elbang_{#1}%
}{ÄθFL}{$\equielbang{\forceL}$}{
  Equil. elbow angle
}

\hidenextnotation
\newnotationcommand{\tarelbang}{\elbang^\ast}{Äθ*}{$\tarelbang$}{
  Target elbow angle
}
...
\hidenextnotation
\newnotationcommand{\act}{\beta}{Äβ}{$\act$}{
  Muscle activation
}

\hidenextnotation
\newnotationcommand{\actX}{\act_X}{ÄβX}{$\actX$}{
  Muscle activation
}

\hidenextnotation
\newnotationcommand{\actT}{\act_\mathrm{T}}{ÄβT}{$\actT$}{
  Triceps activation
}

\hidenextnotation
\newnotationcommand{\actB}{\act_\mathrm{B}}{ÄβB}{$\actB$}{
  Biceps activation
}
...
\newnotationcommandoptarg{2}{\actdomainintp}{%
  D_{\equielbang{#2}}^{\sparse\ifnotempty{#1}{,#1}}%
...
\hidenextnotation
\newnotationcommandoptarg{1}{\optnormpolicyref}{%
  \hat{\policy}^{\opt,\reference\ifnotempty{#1}{,#1}}%
}{p^opttref}{$\optnormpolicyref_t$}{
  Reference normalized optimal policy fcn.
}
...
\newcommand*{\optnormpolicyentryref}[1][]{%
  \hat{\policyentry}^{\opt,\reference\ifnotempty{#1}{,#1}}%
}
...
\hidenextnotation
\newnotationcommandoptarg{1}{\normcetvalueref}{%
  \hat{\tilde{J}}^{\reference\ifnotempty{#1}{,#1}}%
}{Jtref}{$\normcetvalueref_t$}{
  Reference normalized certainty-equivalent-transf.\ value fcn.
}
...
\hidenextnotation
\newnotationcommand{\patience}{\varrho}{Äρ3}{$\patience$}{
  Patience factor
}
...
\hidenextnotation
\newnotationcommand{\sumfcn}{\mathop{\Sigma}}{ÄΣ}{$\sumfcn$}{
  Sum function $\sumfcn(\*x) := \tr{\*1} \*x$
}

\hidenextnotation
\newnotationcommand{\compmult}{\odot}{Ö.}{$\*a \compmult \*b$}{
  Component-wise product of vectors $\*a$ and $\*b$
}

\hidenextnotation
\newnotationcommand{\normcropfactor}{\hat{\beta}}{Äβ2}{$\normcropfactor$}{
...
\hidenextnotation
\newnotationcommand{\multiplier}{\mu}{Äμ}{$\multiplier$}{
...
}{ÄεtEuL2}{$\weightedeulererrorLtwo_t$}{
