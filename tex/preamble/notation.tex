...
\newnotationcommand{\state}{\*x}{xt}{$\state_t$}{
  State variables $\in \clint{\*0, \*1}$
}

\hidenextnotation
\newnotationcommand{\stateentry}{x}{xtj}{$\stateentry_{t,j}$}{
  $j$-th state variable $\in \clint{0, 1}$
  ($j = 1, \dotsc, d$)
}

\hidenextnotation
\newnotationcommand{\policy}{\*p}{pt}{$\policy_t$}{
  Policy variables $\in \real^{m_{\policy}}$
}

\hidenextnotation
\newnotationcommand{\policyentry}{p}{ptj}{$\policyentry_{t,j}$}{
  $j$-th policy variable $\in \real$
  ($j = 1, \dotsc, m_{\policy}$)
}

\hidenextnotation
\newnotationcommand{\stochastic}{\*\omega}{Äωt}{$\stochastic_t$}{
  Stochastic variables $\in \stochdomain$
}
...
\newnotationcommand{\discrstate}{\*\theta}{Äθt}{$\discrstate_t$}{
  Discrete state variables $\in \discrstdomain$
}
...
\hidenextnotation
\newnotationcommand{\valueintp}{J^\sparse}{Jts}{$\valueintp_t$}{
  Interpolated value function
  $\valuefcn_t\colon \clint{\*0, \*1} \to \real$
}

\hidenextnotation
\newnotationcommand{\optpolicyintp}{%
  \policy^{\opt,\sparse}%
}{poptts}{$\optpolicyintp_t$}{
  Interpolated optimal policy function
  $\optpolicyintp_t\colon \clint{\*0, \*1} \to \real^{m_{\policy}}$
}
...
\hidenextnotation
\newnotationcommand{\utilityfcn}{u}{uct}{$\utilityfcn(\consume_t)$}{
