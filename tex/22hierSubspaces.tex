...
\begin{equation}
  \fgset{l}
  = \bigdotcup_{l'=0}^l \{\gp{l',i'} \mid i' \in I_{l'}\},\quad
  I_{l'} :=
  \begin{cases}
...
\begin{figure}
  ...
  \caption{%
    ...
    whose grid points $\gp{l',i'}$ have odd indices $i' \in I_{l'}$
...
\begin{figure}
  \subcaptionbox{%
    Basis functions $\basis{l',i'}^1$ ($l' \le l$, $i' \in I_{l'}$)
...
\begin{lemma}[univariate hierarchical splitting characterization]
  ...
  \begin{itemize}
    \item
    The hierarchical subspaces $W_{l'}$ ($l' \le l$) are subspaces of $\ns{l}$.
    
    \item
    The hierarchical functions
    $\basis{l',i'}$ ($l' \le l$, $i' \in I_{l'}$) are linearly independent.
  \end{itemize}
\end{lemma}
\begin{proof}
  The first condition is equivalent to $\sum_{l'=0}^l W_{l'} \subset \ns{l}$.
...
\begin{proof}
  ...
  over $l$:
  If a linear combination of $\basis{l',i'}^1$ ($l' \le l$, $i' \in I_{l'}$)
  vanishes everywhere, then the coefficients of level $l$ must be zero,
  as otherwise the basis functions $\basis{l,i'}^1$ ($i' \in I_l$) would
...
\begin{equation}
  W_\ßl
  := \spn\{\basis{\ßl,\ßi} \mid \ßi \in I_\ßl\},\quad
  I_\ßl
...
\begin{lemma}[multivariate hierarchical splitting characterization]
  ...
  \begin{itemize}
    \item
    The hierarchical subspaces $W_{\ßl'}$ ($\ßl' \le \ßl$) are subspaces of $\ns{\ßl}$.
    
    \item
    The basis functions $\basis{\ßl',\ßi'}$ ($\ßl' \le \ßl$, $\ßi' \in I_{\ßl'}$)
