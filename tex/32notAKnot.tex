...
The resulting spline space $S_{\ß\xi_l^{p,\notaknot}}^p$ is a subspace
of $S_l^p$ with the correct dimensionality:
\begin{equation}
  \dim \bigoplus_{l'=0}^l W_l^p
  = 2^l + 1
  = \dim S_{\ß\xi_l^{p,\notaknot}}^p.
\end{equation}
The new space $S_{\ß\xi_l^{p,\notaknot}}^p$ satisfies our two requirements:
First, the spline interpolation domain of $S_{\ß\xi_l^{p,\notaknot}}^p$
equals the whole interval.
This means that the Schoenberg--Whitney conditions are satisfied
for $S_{\ß\xi_l^{p,\notaknot}}^p$, since all interpolation grid
(grid points) lie within $D_{\ß\xi_l^{p,\notaknot}}^p = [0, 1]$.
Second, $S_{\ß\xi_l^{p,\notaknot}}^p$ still contains all polynomials of
degree $\le p$, as we have only removed knots compared to $S_l^p$.
...
and contained in the space $S_{\ß\xi_l^{p,\notaknot}}^p$.

\todo{introduce Slp, Slpnak, ... --> change Sec. 3.1}

\todo{write about implementation: which basis function change?}
...
a subspace of $S_l^p$ with the right dimension.
