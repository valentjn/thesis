...
which is a proper subset of the domain $\clint{0, 1}$ (for $p > 1$).
...
since the ``approximation power'' of splines should be better than
that of polynomials.
...
\begin{equation}
  ...
  m := 2^l + p.
\end{equation}
We have $\spldomain{\knotseq}{p} = \clint{0, 1}$ for this choice of $\knotseq$
...
$\frac{\diff^3}{\dx^3} s$ is continuous at the first and at the last
