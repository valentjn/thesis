...
we have $S_l^p = \bigoplus_{l'=0}^l \hs{l'}^p|_{D_l^p}$
with $p = 3$.
...
\begin{proposition}[Schoenberg--Whitney conditions]
  ...
  $s = \sum_{k=0}^{m-1} \interpcoeff{k} b_{k,\knotseq}^p$ for arbitrary data if and only if
  \begin{equation}
...
to find a solution.
\newgsymbol{Slp01}{$S_l^{p,\clint{0,1}}$}{%
...
on the grid of level $l$, i.e., the space $S_\knotseq^p$ with
\begin{equation}
...
Clearly, the sum $\bigoplus_{l'=0}^l \hs{l}^p$ is a subspace of $S_l^{p,\clint{0,1}}$,
...
so-called \term{not-a-knot boundary conditions} into $S_l^{p,\clint{0,1}}$.
...
\begin{subequations}
  ...
  \end{gather}
\end{subequations}
This knot sequence $\knotseq_l^{p,\notaknot}$
...
(grid points) lie within $D_{\knotseq_l^{p,\notaknot}}^p = \clint{0, 1}$.
...
\begin{figure}
  ...
  \caption{%
    Not-a-knot knot sequence $\knotseq_l^{p,\notaknot}$
    ...
    The spline interpolation domain $D_{\knotseq_l^{p,\notaknot}}^p$
    \emph{(thick line)}
...
uniform knot sequence $\knotseq_l^p$ of \thmref{cor:nodalBSplineSpace}
...
(because of the symmetry of $\knotseq_l^{p,\notaknot}$),
...
\begin{subequations}
  \begin{gather}
    ...
    \begin{cases}
      ...
      l < \lceil\log_2(p+1)\rceil,\\
      b_{i,\knotseq_l^{p,\clenshawcurtis,\notaknot}}^p,&
      l \ge \lceil\log_2(p+1)\rceil,
    \end{cases}\quad
    l \in \natz,\quad
    i \in \hiset{l},\\
