...
$\rspldomain{l}{p} =
\clint{\ms{l} \tfrac{p-1}{2},\; 1 - \ms{l} \tfrac{p-1}{2}}$,
...
However, one can clearly see deviations of the interpolant from the polynomial
near the boundary, where the relative error exceeds \SI{10}{\percent}.
The oscillations visibly occur even in the interior of the
spline interpolation domain $\rspldomain{l}{p}$.
Of course, this phenomenon impairs the approximation quality
...
\begin{figure}
  ...
  \caption[%
    ...
    fail to replicate the cubic polynomial
...
with $p = 3$.
Since cubic polynomials as $\objfun$ are also cubic splines,
...
the points $x = 0$ and $x = 1$ are not contained in~$\rspldomain{l}{p}$.
...
$\clint{\gp{l,2^l-2}, 1}$ \cite{Hoellig13Approximation}.
...
\end{equation}
In turn, this is equivalent to removing these knots
...
\begin{figure}
  ...
  \hfill%
  \begin{tikzpicture}
    \draw[decorate,thick,decoration={brace,aspect=0.145}] (0,-8.5) -- (0,0);
...
we only have to implement a number of special cases for coarse levels $l$.
...
can be replicated exactly with hierarchical natural B-splines.
