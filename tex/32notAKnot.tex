...
\begin{proposition}[Schoenberg--Whitney conditions]
  ...
  $s = \sum_{k=0}^{m-1} \interpcoeff{k} b_{k,\ß\xi}^p$ for arbitrary data if and only if
  \begin{equation}
...
\begin{figure}
  ...
  \caption{%
    ...
    B-splines $\basis{l',i'}^{p,\notaknot}$ and
    $\basis{l',i'}^{p,\clenshawcurtis,\notaknot}$
    ($l ' \le l$, $i' \in \his{l'}$, $p = 3$) up to level $l = 3$
