...
\begin{figure}
  \subcaptionbox{%
    ...
    $\bspl{l',1}{p}{\modified}$ (\emph{dashed}, $l' \ge 2$)
  ...
  \hfill%
  \subcaptionbox{%
    Modified hierarchical cubic B-splines $\bspl{l',i'}{p}{\modified}$
...
\usenotation{.cc}
The so-called Chebyshev points $\gp{l,i}^\cc$ and the
...
The \term{Chebyshev points} $\gp{l,i}^\cc$ of level $l \in \natz$
are defined as
\begin{equation}
  \gp{l,i}^\cc
...
\begin{subequations}
  ...
  \begin{gather}
    ...
    :=
    \begin{cases}
      (k - \frac{p+1}{2}) \cdot \gp{l,1}^\cc,&
      k = 0, \dotsc, \frac{p-1}{2},\\
      \gp{l,k-(p+1)/2}^\cc,&
      k = \frac{p+1}{2}, \dotsc, 2^l + \frac{p+1}{2},\\
      1 + (k - 2^l - \frac{p+1}{2}) \cdot \gp{l,1}^\cc,&
...
the Chebyshev points $\gp{l,i}^\cc$
...
\begin{figure}
  ...
  \hfill%
  \subcaptionbox{%
    Sparse Clenshaw--Curtis grid $\regsgset{n}{d}{\cc}$
