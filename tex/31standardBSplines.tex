...
$\varphi_{l,i}^p\colon [0, 1] \to \RR$ of level~$l \in \NNz$ and index
...
by $\supp \varphi_{l,i}^p = [0, 1] \cap [\gp{l,i-(p+1)/2}, \gp{l,i+(p+1)/2}]$.
...
$\varphi_{l,i}^p$ ($l \in \NNz$, $i \in I_l$).

\begin{figure}
  \subcaptionbox{%
    Nodal B-splines $\varphi_{l,i}^p$ ($i \in I_l$) and grid points $\gp{l,i}$
    \emph{(dots)}.%
  }[70mm]{%
...
$\varphi_{l,i}^p$ coincide with the grid points \cite{Valentin14Hierarchische}
\begin{equation}
  \gp{l,i-(p+1)/2},\quad
  \dotsc,\quad
  \gp{l,i},\quad
...
\begin{equation}
  \gp{l,i-p/2} - \frac{h_l}{2},\quad
  \dotsc,\quad
  \gp{l,i} - \frac{h_l}{2},\quad
  \gp{l,i} + \frac{h_l}{2},\quad
  \dotsc,\quad
  \gp{l,i+p/2} + \frac{h_l}{2}.
\end{equation}
As we will see,
...
\begin{equation}
  ...
  - \frac{1}{h_l} \sum_{i \in \ZZ} \gp{l,i} \varphi_{l,i}^p(x)
...
$\varphi_{l,1}^p$ left of its grid point $\gp{l,1}$ \cite{Pflueger10Spatially}.
...
\begin{figure}
  ...
  \subcaptionbox{%
    ...
    ($l' = 1, \dotsc, l$, $i' \in I_{l'}$) and grid points $\gp{l',i'}$
    \emph{(dots)}.%
  }[75mm]{%
...
than the standard equidistant choice of $\gp{l,i} = i \cdot h_l$,
...
$\varphi_{l,i}^{p,\clenshawcurtis}$ of level $l \in \NNz$ and index
