...
\begin{figure}
  \subcaptionbox{%
    Nodal B-splines $\basis{l,i}^p$ ($i \in I_l$) and grid points $\gp{l,i}$
    \emph{(dots)}.%
  }[70mm]{%
  ...
  \hfill%
  \subcaptionbox{%
    Hierarchical B-splines $\basis{l',i'}^p$ ($l' \le l$, $i' \in I_{l'}$)
...
$\basis{l,i}^p$ ($i \in I_l$) are linearly independent on $\clint{0, 1}$.
