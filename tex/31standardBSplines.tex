...
$\varphi_{l,1}^p$ left of its grid point $x_{l,1}$ \cite{Pflueger10Spatially}.
...
The \term{Chebyshev points} $x_{l,i}^\clenshawcurtis$ of level $l \in \NNz$
are defined as
\begin{equation}
...
(i.e., there are $2^l + 1$ points $x_{l,i}^\ast$ in each level $l \in \NNz$)
and they are nested
