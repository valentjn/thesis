...
\begin{definition}[non-uniform B-splines]
  ...
  \newgsymbol{bkxip}{$b_{k,\knotseq}^p$}{%
    Non-uniform B-spline of degree $p$ with knots $\knotseq$ and index $k$%
  }%
  For $k = 0, \dotsc, m - 1$,
  the \term{(non-uniform) B-spline} $b_{k,\knotseq}^p$ of degree $p$
  ...
  \begin{equation}
    ...
    :=
    \begin{cases}
      \dfrac{x - \knot{k}}{\knot{k+p} - \knot{k}} b_{k,\knotseq}^{p-1}(x) +
      \dfrac{\knot{k+p+1} - x}{\knot{k+p+1} - \knot{k+1}}
      b_{k+1,\knotseq}^{p-1}(x),&p \ge 1,\\
...
\begin{proposition}[spline space]
  ...
  \newgsymbol{Sxip}{$S_\knotseq^p$}{%
    Spline space of degree $p$ with knots $\knotseq$%
  }%
  Let $\knotseq = (\knot{0}, \dotsc, \knot{m+p})$ be a knot sequence.
  Then, the B-splines $b_{k,\knotseq}^p$ ($k = 0, \dotsc, m - 1$)
  form a basis of the \term{spline space}
  \begin{equation}
    S_\knotseq^p
    := \spn\{b_{k,\knotseq}^p \mid k = 0, \dotsc, m - 1\}.
  \end{equation}
  $S_\knotseq^p$ contains exactly those functions that are continuous
  on $D_\knotseq^p := \clint{\knot{p}, \knot{m}}$,
  polynomials of degree $\le p$ on every knot interval
  $\clint{\knot{k}, \knot{k+1}}$ \todo{check half-open interval?} in $D_\knotseq$
...
\begin{figure}
  ...
  \caption{%
    ...
    $b_{k,\knotseq}^p$ ($k = 0, \dotsc, m - 1$, $p = 3$).
    ...
    $b_{m-1,\knotseq}^p$,
    the B-splines span the spline space $S_\knotseq^p$.
    ...
    $s = \sum_{k=0}^{m-1} \interpcoeff{k} b_{k,\knotseq}^p$
    of the B-splines.%
  }%
...
\begin{figure}
  ...
  \caption{%
    Uniform knot sequence $\knotseq_l^p$ \emph{(ticks on horizontal axis)}
    ...
    The spline interpolation domain $D_l^p := D_{\knotseq_l^p}^p$
    \emph{(thick line)}
...
\begin{corollary}[nodal B-spline space]
  ...
  \newgsymbol{xilp}{$\knotseq_l^p$}{%
    Knot sequence
    for uniform nodal B-splines of level $l$, degree $p$%
  }%
  \newgsymbol{Slp}{$S_l^p$}{%
    Spline space $:= S_{\knotseq_l^p}^p$
  ...
  ($i = 0, \dotsc, 2^l$)
  of level $l \in \natz$ are
  a basis of the spline space $S_{\knotseq_l^p}^p$, where
  \begin{gather}
...
\begin{proof}
  We have $\basis{l,i}^p = b_{i,\knotseq_l^p}^p$ for
  $i = 0, \dotsc, m - 1$,
...
\begin{equation}
  ...
  := b_{i,\knotseq_l^{p,\clenshawcurtis}}^p,
\end{equation}
where
\begin{subequations}
  ...
  \end{gather}
\end{subequations}
For the construction of the knot sequence $\knotseq_l^{p,\clenshawcurtis}$,
