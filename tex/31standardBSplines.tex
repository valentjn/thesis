...
The multivariate case is treated as usual by tensor products.
...
\begin{figure}
  ...
  \caption[%
    ...
    \lefthphantom{5.}{5.}\,
    $\frac{\diff}{\dx} \cardbspl{p}$ \emph{(dashed)}
...
\begin{figure}
  ...
  \hfill%
  \begin{tikzpicture}
    \draw[decorate,thick,decoration={brace,aspect=0.153}] (0,-8.5) -- (0,0);
...
that fundamental splines would not be defined at all
(as we will see in \cref{sec:443fundamentalSplines}).
Therefore, and
as the odd degrees include the hat function case~($p = 1$) and the
...
\begin{equation}
  ...
  \frac{\diff^2}{\dx^2} \bspl[\modified]{l,i}{p}(x) = 0,\quad
...
\begin{lemma}[Marsden's identity]
  ...
  \begin{equation}
    ...
    (x - y)^p
    = \sum_{k=0}^{m-1}
    \;\bigl[(\knot{k+1} - y) \dotsm (\knot{k+p} - y)\bigr]\;
    \nonunifbspl{k,\knotseq}{p}(x).
  \end{equation}
\end{lemma}
...
By comparing the coefficients of $y^p$ and $y^{p-1}$ of
both sides of Marsden's identity,
we obtain a possible definition for $\bspl[\modified]{l,i}{p}$:
\begin{equation}
...
$\bspl[\cc,\modified]{l,i}{p}$ can be defined using the
