...
\begin{figure}
  ...
  \caption{%
    ...
    the nodal space $\restrictspace{\ns{l}^p}{D_l^p}$ decomposes into the direct sum
    of the hierarchical subspaces $\restrictspace{\hs{l'}^p}{D_l^p}$ ($l' \le l$).%
  }
...
\begin{figure}
  ...
  \caption{%
    ...
    Elements of this space are splines $\spl\colon D_\knotseq^p \to \real$
    \emph{(black line)},
...
\begin{corollary}[nodal B-spline space]
  ...
  The restricted nodal B-splines $\restrictfcn{\basis{l,i}^p}{D_l^p}$
  ($i = 0, \dotsc, 2^l$)
  of level $l \in \natz$ are
...
\begin{lemma}[first condition of \cref{lemma:hierSplittingUV}]
  ...
  $\restrictspace{\hs{l'}^p}{D_l^p}$ ($l' \le l$) are
...
\begin{proof}
  ...
  $\restrictspace{\hs{l'}^p}{D_l^p}$, we can conclude
  $\restrictspace{\hs{l'}^p}{D_l^p} \subset \restrictspace{\ns{l}^p}{D_l^p}$.
\end{proof}
...
$\restrictspace{\hs{l-1}^p}{D_l^p}$ is not contained in $\restrictspace{\ns{l}^p}{D_l^p}$
for even degrees $p$.
...
\begin{corollary}[hierarchical splitting for uniform B-splines]
  ...
  \begin{equation}
    \restrictedsplspace{l}{p}
    = \restrictspace{\ns{l}^p}{D_l^p}
    = \bigoplus_{l'=0}^l \restrictspace{\hs{l'}^p}{D_l^p}.
  \end{equation}
\end{corollary}
