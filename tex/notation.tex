...
\newnotationcommand[1]{\volcell}{\mathrm{vol}(#1)}{volx}{$\volcell{\*x}$}{
  Volume fraction of the material in a micro-cell with parameter $\*x$
}

\newnotationcommand{\volfracub}{r^\ast}{r*}{$\volfracub$}{
  Upper bound on the volume fraction
  $\voldens{\densfcn}{\domain}/\vol{\domain}$
}

\newnotationcommand{\etensor}{\mat{E}}{E}{$\etensor$}{
  Elasticity tensor in
  $\real^{3 \times 3}$ (2D) or in
  $\real^{6 \times 6}$ (3D)
}
