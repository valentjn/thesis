...
\newnotation{n}{$n$}{
  Level $\in \NNz$ of full or sparse grid
}
...
{$\opint{\ßa, \ßb}$}{
  ...
  with $\opint{a_t, b_t} := \{x_t \in \RR \mid a_t < x_t < b_t\}$
}
...
{$\hopint{\ßa, \ßb}$}{
  ...
  with $\hopint{a_t, b_t} := \{x_t \in \RR \mid a_t \le x_t < b_t\}$
}
...
\newnotationcommand{\levelset}{L}{L}{$\levelset$}{
  Finite subset $L \subset \NNz^d$ of levels
}
...
\newnotationcommand[4]{\coarselevelset}{%
  ...
  Finite subset $L \subset \NNz^d$ of levels for the grid
...
{$\boundary{\Omega}$}{
  Topological boundary of a set $\Omega \subset \RR^d$
}
