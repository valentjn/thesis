...
\newnotationcommand{\dimdomain}{{\tilde{d}}}{d~}{$\dimdomain$}{%
  Dimensionality of $\domain$%
}

\newnotationcommand{\domain}{\tilde{\Omega}}{ÄΩ~}{$\domain$}{%
  Domain $\domain \subset \real^\dimdomain$%
}

\newnotationcommand{\densfcn}{\varrho}{Äρ}{$\densfcn$}{%
  Density function $\densfcn\colon \domain \to \clint{0, 1}$%
}

\newnotationcommand{\compliance}{J}{Jρ}{$\compliance(\densfcn)$}{%
  Compliance of the density function $\densfcn$%
}
...
\newnotationcommand[2]{\voldens}{\mathrm{vol}_{#1}(#2)}{volρΩ}{%
  ...
  Volume of $\domain \subet \real^\dimdomain$ with respect to the
  density function $\densfcn$%
}

\newnotationcommand{\volfracub}{r^\ast}{r*}{$\volfracub$}{%
  Upper bound on the volume fraction
  $\voldens{\densfcn}{\domain}/\volume{\domain}$%
}
