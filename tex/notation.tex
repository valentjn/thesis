...
\newnotationcommand[1]{\upopmv}{\linop^{(#1)}}{Lt1}{%
  $\upopmv{t_1, \tosc, t_j}$%
}{
  Operator $\upopmv{t_1, \dotsc, t_j}\colon \real^{\ngp} \to \real^{\ngp}$
...
\newcommand*{\upopmvinv}[1]{(\upopmv{#1})^{-1}}

\newnotationcommand[1]{\upopuv}{\linop^{(#1),\mathrm{1D}}}{Lt1D}{%
  $\upopuv{t}$%
}{
  One-dimensional operator $\upopuv{t}\colon
...
\let\simorig\sim
\let\sim\undefined
\newnotationcommand{\sim}{\simorig}{Ösim}{$\sim$}{
  Equivalence relation where
  $a \sim b$ denotes that two elements $a, b$ are equivalent
}

\newnotationcommand[1]{\samepole}{\sim_{#1}}{Ösimt}{$\samepole{t}$}{
...
\newnotationcommand[2]{\eqclass}{[#1]_{#2}}{Öeqclass}{%
  ...
  with respect to an equivalence relation $\sim$
}
...
\newnotationcommand{\lisetpole}{\liset_\mathrm{pole}}{Kpole}{$\lisetpole$}{
  Pole $\lisetpole \in \eqclasses{\liset}{\samepole{t}}$
  in some dimension $t$
  ($\lisetpole$ is a subset of $\liset$)
}
...
\newnotationcommandoptarg{2}{\parentfundspl}{%
  ...
  Fundamental spline of degree $p$
}
