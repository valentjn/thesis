...
\newnotationcommand[3]{\hierbspl}{%
  \varphi_{#1}^{#2\appendwithcomma{#3}}%
}{philip}{$\hierbspl{\ßl,\ßi}{\ßp}{}$}{
...
\newnotationcommand{\liset}{K}{K}{$\liset$}{
  ...
  or finite subset of continuous indices $\ßk \in \NNz^d$
}
...
\newnotationcommand{\linop}{L}{L}{$\linop$}{
  Linear operator $\linop\colon \real^\ngp \to \real^\ngp$
  on grid point data
}

\newnotationcommand{\vlinin}{\ßu}{u}{$\linin$}{
  Input of the linear operator $\linop$
}

\newcommand*{\linin}[1]{u_{#1}}

\newnotationcommand{\vlinout}{\ßy}{y}{$\linout$}{
