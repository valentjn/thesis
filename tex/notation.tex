...
\newnotation{\knot}{\xi}{xi}{$\knot{k}$}{
  Knot $\in \real$ of a spline space
  (point where the splines are not smooth)
}

\newnotation{\knotseq}{\ß\xi}{xi!}{$\knotseq$}{
  Knot sequence $:= (\knot_0, \dotsc, \knot_{m+p})$
}

\newnotation[3]{\nodalknot}{%
  \xi_{#1}^{#2\appendwithcommand{#3}}%
}{xilkp}{$\nodalknot{l,k}{p}{}$}{
