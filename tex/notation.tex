...
\newnotationcommand[3]{\interiorregsgset}{%
  \interior{\Omega}_{#1,#2}^{%
    \sparse%
    \if\relax\detokenize{#3}\relax\else,#3\fi%
  }%
}{Omegandso}{$\interiorregsgset{n}{d}{}$}{
  Set $:= \regsgset{n}{d}{} \cap \openinterval{\ß0, \ß1}$ of
  interior regular sparse grid point of level $n$ with dimensionality $d$
}

%\newnotationcommand[4]{\coarseregsgspace}{%
%  V_{#1,#2}^{%
%    \sparse(#3)%
%    \if\relax\detokenize{#4}\relax\else,#4\fi%
%  }%
%}{Vndsb}{$\coarseregsgspace{n}{d}{b}{}$}{
%  Regular sparse grid space of level $n$ with dimensionality $d$
...
\newnotationcommand{\sgspace}{V^\sparse}{Vs}{$\sgspace$}{
  Arbitrary sparse grid space (possibly spatially adaptive)
}

\newnotationcommand{\sgset}{\Omega^\sparse}{Omegas}{$\sgset$}{
