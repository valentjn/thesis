...
\newnotationcommand[2]{\restrictfcn}{#1|_{#2}}{f|D}{$\restrictfcn{f}{D}$}{
  Restriction $\restrictfcn{f}{D}\colon D \to \real$,
  $\restrictfcn{f}{D}(x) := f(x)$,
  onto a sub-domain $D$ for a function $f$
}

\newnotationcommand[2]{\restrictspace}{#1|_{#2}}{V|D}%
{$\restrictspace{V}{D}$}{
  Restriction $:= \{\restrictfcn{f}{D} \mid f \in V\}$ onto a sub-domain $D$
...
\makecommandnotation{\deg}{deg}{$\deg f$}{
